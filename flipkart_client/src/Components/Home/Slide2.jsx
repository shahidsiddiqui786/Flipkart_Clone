...
import { timerURL } from '../../constant/data';
                    ...
                    data.map(temp => (
                        <Link to={`product/${temp.id}`} style={{textDecoration: 'none'}}>
                            <Box textAlign="center" style={{ padding: '25px 15px' }}>
                                <Image src={temp.thumbnail} />
                                <Text style={{ fontWeight: 600, color: '#212121' }}>{temp.title}</Text>
                                <Text style={{ color: 'green' }}>Min {temp.discountPercentage} % Off</Text>
                                <Text style={{ color: '#212121', opacity: '.6' }}>{temp.category}</Text>
                            </Box>
                        </Link>   
                    ))
                }
